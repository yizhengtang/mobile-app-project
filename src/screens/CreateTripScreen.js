  ...
  return (
    <View style={styles.stepContent}>
      ...
      <View style={styles.reviewCard}>
        <ReviewRow icon="location-outline" label="Destination" value={data.destination || '—'} />
        <ReviewRow icon="calendar-outline" label="Dates" value={`${data.startDate || '—'} → ${data.endDate || '—'}`} />
        <ReviewRow icon="home-outline" label="Accommodation" value={data.accommodation || '—'} />
      ...
      <View style={styles.aiNotice}>
        ...
        <Text style={styles.aiNoticeText}>
          The AI will generate a day-by-day itinerary using your inputs. This may take a few seconds.
        </Text>
      </View>
    </View>
  );
...
export default function CreateTripScreen({ navigation }) {
  ...
  function validateStep() {
    if (step === 1) {
      ...
      if (!formData.startDate.match(/^\d{4}-\d{2}-\d{2}$/)) return 'Start date must be YYYY-MM-DD.';
      if (!formData.endDate.match(/^\d{4}-\d{2}-\d{2}$/)) return 'End date must be YYYY-MM-DD.';
      if (new Date(formData.endDate) <= new Date(formData.startDate)) return 'End date must be after start date.';
    }
    if (step === 3) {
      if (formData.transportModes.length === 0) return 'Select at least one transport mode.';
    }
    return null;
  }
  ...
  function handleSubmit() {
    const [city, ...rest] = formData.destination.split(',');
    const newTrip = addTrip({
      name: formData.name.trim(),
      destination: { city: city.trim(), country: rest.join(',').trim() || '' },
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={() => { setFormData(INITIAL_DATA); setStep(1); }}>
