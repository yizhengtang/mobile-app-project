...
export default function ProfileScreen() {
  ...
  const totalDays = readyTrips.reduce((sum, t) => sum + t.days.length, 0);
  const totalStops = readyTrips.reduce(
    (sum, t) => sum + t.days.reduce((dSum, d) => dSum + d.stops.length, 0),
    0
  );
    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View style={styles.avatarSection}>
          ...
          <Text style={styles.email}>Sign in to sync your trips across devices</Text>
...
const styles = StyleSheet.create({
  ...
  email: { fontSize: 13, color: COLORS.textSecondary, marginTop: 4, marginBottom: 16 },
  signInBtn: {
  ...
  signInBtnText: { fontSize: 14, fontWeight: '600', color: COLORS.primary },
  statsRow: {
    flexDirection: 'row',
    gap: 10,
